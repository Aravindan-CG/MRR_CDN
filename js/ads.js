var gameInput = { gameName: 'MRRacer', publisherName: 'ChennaiGames' };
...
if(!window.sendCustomAnalyticsEvent) {
...
var LPBannerInstance, LBBannerInstance, StickyBannerInstance, replayInstance, GlanceGamingAdInstance, rewardInstance, interstitialInstance, _triggerReason;
var is_replay_noFill = false
var is_rewarded_noFill = false
var isRewardGranted = false
var isRewardedAdClosedByUser = false
var is_interstitial_noFill = false
...
var interstitialObj = {
    adUnitName: `${gameInput.publisherName}_${gameInput.gameName}_FsInterstitial_Interstitial`,
    placementName: isTestModeOn ? "Test_Interstitial" : "FsInterstitial",
    pageName,               //Game Name
    categoryName,         //Publisher Name
    containerID: '',
    height: '',
...
function interstitialCallbacks(obj)
{
    obj.adInstance?.registerCallback('onAdLoadSucceed', (data) => {
        console.log('onAdLoadSucceeded Interstitial CALLBACK', data);
        if (obj.adUnitName === interstitialObj.adUnitName) {
            is_interstitial_noFill = false
        }
    });

    obj.adInstance?.registerCallback('onAdLoadFailed', (data) => {
        console.log('onAdLoadFailed Interstitial CALLBACK', data);
        if (obj.adUnitName === interstitialObj.adUnitName) {
            is_interstitial_noFill = true
        }
    });

    obj.adInstance?.registerCallback('onAdDisplayed', (data) => {
        console.log('onAdDisplayed Intestitial CALLBACK', data);
    });

    obj.adInstance?.registerCallback('onAdClosed', (data) => {
        console.log('onAdClosed Interstitial CALLBACK', data);
        runOnAdClosed();
    });

    obj.adInstance?.registerCallback('onAdClicked', (data) => {
        console.log('onAdClicked Interstitial CALLBACK', data);
    });
}
...
function interstitialEvent()
{
    ...
    if(!is_interstitial_noFill)
    {
        window.GlanceGamingAdInterface.showInterstitialAd(interstitialInstance);
    }
    else
    {
        runOnAdClosed();
